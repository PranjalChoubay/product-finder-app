...
def search():
    try:
        ...
        # --- CHANGE 1: Switched to DummyJSON API to get ALL products ---
        response = requests.get("https://dummyjson.com/products?limit=0")
        # --- CHANGE 2: Extracted the list of products from the response object ---
        data = response.json().get("products", [])
        
        # Split query into terms for better matching
        query_terms = re.findall(r'\b\w+\b', query.lower())
        
        # Enhanced search across multiple fields (Your original logic is preserved)
        results = []
        for product in data:
            title = product.get("title", "").lower()
            description = product.get("description", "").lower()
...
def search_suggestions():
    ...
    try:
        ...
        # Extract unique categories and common words from titles (Your original logic is preserved)
